...
curl -sSL https://github.com/sharkdp/fd/releases/download/v${FD_VER}/fd-v${FD_VER}-i686-unknown-linux-musl.tar.gz | tar xz -C /usr/local/bin && \
    ln -s /usr/local/bin/fd-v${FD_VER}-i686-unknown-linux-musl/fd /usr/local/bin/fd && \
curl -sSL https://github.com/chmln/sd/releases/download/v${SD_VER}/sd-v${SD_VER}-x86_64-unknown-linux-musl > /usr/local/bin/sd && chmod +x /usr/local/bin/sd
